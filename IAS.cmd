...
::============================================================================
::
::   IDM Activation Script (IAS) - Complete with All Features
::
::   Features: All original checks, advanced activation, and firewall blocking.
::
...
:_reset
cls
if not %HKCUsync%==1 (if not defined terminal mode 153, 35) else (if not defined terminal mode 113, 35)
if not defined terminal %psc% "&%_buf%" %nul%
echo:
!idmcheck! && taskkill /f /im idman.exe %nul%

:: --> [MOD] Unblock firewall
...
!idmcheck! && (echo: & taskkill /f /im idman.exe %nul%)
...
if not defined _fileexist (
  %eline%
  echo Error: Unable to download files with IDM to create registry keys.
  goto :done
)

%psc% "$sid = '!_sid!'; $HKCUsync = !HKCUsync!; $lockKey = 1; $deleteKey = $null; $f=[io.file]::ReadAllText('!_batp!') -split ':regscan\:.*';iex ($f[1])"

:: --> [MOD] Block firewall and disable updates
...
if defined terminal (call :_color %_Yellow% "Press 0 key to return..." & choice /c 0 /n) else (call :_color %_Yellow% "Press any key to return..." & pause %nul1%)
goto MainMenu

:done2
if %_unattended%==1 timeout /t 2 & exit /b
if defined terminal (echo Press 0 key to exit... & choice /c 0 /n) else (echo Press any key to exit... & pause %nul1%)
exit /b
...
for /f "delims=" %%a in ('%psc% "$key = -join ((Get-Random -Count 20 -InputObject ([char[]]('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'))));$key = ($key.Substring(0, 5) + '-' + $key.Substring(5, 5) + '-' + $key.Substring(10, 5) + '-' + $key.Substring(15, 5));Write-Output $key"') do set key=%%a
...
:add
if "!errorlevel!"=="0" (set "reg=%reg:"=%" & echo Added - !reg!) else (set "reg=%reg:"=%" & call :_color2 %Red% "Failed - !reg!")
exit /b
...
:_color
if %_NCS% EQU 1 (echo %esc%[%~1%~2%esc%[0m) else (%psc% write-host -back '%1' -fore '%2' '%3')
exit /b

:_color2
if %_NCS% EQU 1 (echo %esc%[%~1%~2%esc%[%~3%~4%esc%[0m) else (%psc% write-host -back '%1' -fore '%2' '%3' -NoNewline; write-host -back '%4' -fore '%5' '%6')
exit /b
